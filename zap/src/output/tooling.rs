...
impl<'src> ToolingOutput<'src> {
	...
	fn push_event_callback(&mut self, first: bool, ev: &EvDecl) {
		...
		self.push_line("Arguments = { { event_id = id }, value },");
	...
	fn push_function_callback(&mut self, first: bool, fn_decl: &FnDecl) {
		...
		self.push_line("local function_call_id = buffer.readu8(incoming_buff, read(1))");
		...
		self.push_line("Arguments = { { event_id = id, function_call_id = function_call_id }, value },");
	...
	pub fn output(mut self) -> String {
		...
		self.push_line("--#selene: allow(incorrect_standard_library_use)");
